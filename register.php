<?php
/**
 * 注册用户  add by zz@ 2011.11.23  -- start
*/
echo 'abc123';
//新浪微博省份城市
$WB_CITY_ARR = array(
...
$wbUinfo = $c->show_user_by_id($tlid);
require_once("../functions.php");
require_once("../lib/class.Mysql.php");
$newUser['password']  	= generatePassword(8);
$newUser['password']  	= md5($newUser['password']);
$newUser['username']  	= $wbUinfo['screen_name'];
$newUser['email']  		= "weibo_".$wbUinfo['id']."@weibo.com";
$newUser['head_pic'] 	= $wbUinfo['avatar_large'];
$newUser['reg_ip'] 		= getClientIP();
$newUser['connfrom'] 	= WB_APP_CONN_XIXUEGUI;
$newUser['sex'] = $wbUinfo['gender'] == "m" ? 1 : 2;
$newUser['time_created']= time();
$newUser['head_pic']	.=".jpg";
$newUser['province'] 	= isset($WB_CITY_ARR[$wbUinfo['province']]) ? $WB_CITY_ARR[$wbUinfo['province']]['province'] : "";
$newUser['city'] 	= isset($WB_CITY_ARR[$wbUinfo['province']]) ? (isset($WB_CITY_ARR[$wbUinfo['province']]['city'][$wbUinfo['city']]) ? $WB_CITY_ARR[$wbUinfo['province']]['city'][$wbUinfo['city']] : "" ): "";
...
//查询用户
$query = $db->query("select id,head_pic from tb_user where email = '".$newUser['email']."'");
...
if (!$res){//用户存在 更新connect数据
	...
	$db->query("insert into tb_user(username , password , sex , email , head_pic , reg_ip , province , city , connfrom , time_created) values ('".$newUser['username']."','".$newUser['password']."','".$newUser['sex']."','".$newUser['email']."','".$headImgUrl."','".$newUser['reg_ip']."','".$newUser['province']."','".$newUser['city']."','".$newUser['connfrom']."','".$newUser['time_created']."')");
	$uid = $db->insert_id();
	
	$conn['type'] 	= CONNECT_TYPE_SINA;
	$conn['uid'] 	= $uid;
	$conn['connuid'] 	= $wbUinfo['id'];
	$conn['connuname'] 	= $wbUinfo['screen_name'];
	$conn['isbind'] = 1;
	$conn['issync'] = 1;
	$conn['token'] 	= '';
	$conn['token_secret'] = '';
	...
	$db->query("insert into tb_connect (type,uid,connuid,connuname,token,token_secret,isbind,issync,createtime,updatetime) values ('".$conn['type']."','".$conn['uid']."','".$conn['connuid']."','".$conn['connuname']."','".$conn['token']."','".$conn['token_secret']."',0,0,'".$conn['createtime']."','".$conn['updatetime']."')");
	...
	$db->query("update tb_user set username = '".$newUser['username']."' , head_pic = '".$headImgUrl."' where id =".$conn['uid']);
	$db->query("update tb_connect set connuid = '".$conn['connuid']."' , connuname = '".$conn['connuname']."' , token = '".$conn['token']."' , token_secret = '".$conn['token_secret']."' , isbind = 0 , issync = 0 ,updatetime = ".$conn['updatetime']." where type = ".$conn['type']." and uid =".$conn['uid']);
}
