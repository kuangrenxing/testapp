...
class ImageMagick 
{
	...
	function MagickThumb($src_file , $dst_file = '' , $dst_width , $dst_height , $suffix='_thumb' , $mode = 0)
	{
		...
		if(strtolower($info['format'])=='gif') {
			if (ImageMagick::IsAnimation($src_file)) {
				ImageMagick::ResizeGif($info['mw'],$dst_file, $dst_width, $dst_height);return $dst_file;
			}
		}
		switch ($mode)
		{
	...
	function ResizeGif($src_mw , $dst_file , $dst_width, $dst_height) 
	{
		...
		ImageMagick::SaveImage($src_mw, $dst_file, 'gif');
	}
